...
export const ContentStrategy: React.FC = () => {
  ...
  const generateStrategy = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('strategy-planner', {
        body: { goal },
        headers: {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Strategic Goal</h2>
        ...
        <>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="text-center">
                <div className="text-lg font-bold text-blue-600">{strategyData.strategy_metadata.posts_analyzed}</div>
                <div className="text-sm text-gray-500">Posts Analyzed</div>
              </div>
              <div className="text-center">
                <div className={`inline-flex items-center px-2 py-1 rounded-full text-sm font-medium ${
                  getConfidenceColor(strategyData.strategy_metadata.strategy_confidence)
                }`}>
                  {Math.round(strategyData.strategy_metadata.strategy_confidence * 100)}% Confidence
                </div>
                <div className="text-sm text-gray-500 mt-1">Strategy Confidence</div>
              </div>
              <div className="text-center">
                <div className="text-sm text-gray-900">
                  {new Date(strategyData.strategy_metadata.based_on_persona_date).toLocaleDateString()}
                </div>
                <div className="text-sm text-gray-500">Based on Analysis</div>
              </div>
              <div className="text-center">
                <div className="text-sm text-gray-900">
                  {new Date(strategyData.strategy_metadata.created_at).toLocaleDateString()}
                </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              >
                {expandedSection === 'weekplan' ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
              </button>
            </div>
                ...
                <div key={index} className="border border-gray-200 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div>
                      ...
                      <span className="ml-2 text-gray-600">{plan.topic_theme}</span>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <span className="ml-2 text-gray-600">{plan.call_to_action}</span>
                    </div>
                  </div>
                  ...
                  {expandedSection === 'weekplan' && (
                    <div className="mt-3 pt-3 border-t border-gray-200">
                      <div className="text-xs text-gray-600">
                        <div className="mb-1"><strong>Rationale:</strong> {plan.rationale}</div>
                        <div><strong>Evidence:</strong> {plan.evidence_link}</div>
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="text-gray-400 hover:text-gray-600"
              >
                {expandedSection === 'drafts' ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
              </button>
            </div>
                  ...
                  {expandedSection === 'drafts' && (
                    <div className="pt-3 border-t border-gray-200">
                      <div className="text-xs text-gray-600 space-y-1">
                        <div><strong>Rationale:</strong> {draft.rationale}</div>
                        <div><strong>Persona Evidence:</strong> {draft.persona_evidence}</div>
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="text-gray-400 hover:text-gray-600"
              >
                {expandedSection === 'voice' ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {strategyData.voice_guide.map((guide, index) => (
                <div key={index} className="border border-gray-200 rounded-lg p-4">
                  <h3 className="font-medium text-gray-900 mb-2">{guide.principle}</h3>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-amber-800 mt-1">
                  These AI-generated recommendations are based on your content patterns and stated goals. 
                  Please review each suggestion carefully and adapt them to your specific context, brand voice, and current market conditions. 
                  Consider A/B testing different approaches to validate effectiveness.
                </p>
              </div>
            </div>
          </div>
